...
// closegraph();
 ...
 void UserInfo::lock(){
  for (int i = 1; i <= 30; i++){
		settextstyle(8,0,10);
		bgiout<<i;
		outstreamxy((getmaxwidth()/2-100),(getmaxheight()/2)-100);
		settextstyle(8,0,3);
		outtextxy((getmaxwidth()/2-550),(getmaxheight()/2)+100,"Application is locked for 30 seconds close application or wait for 30 seconds");
		delay(1000);
		cleardevice();
	}
 }

 int UserInfo::login(int attempt){
    anime.animation2();
    fstream f; 
    f.open("password.dat");
